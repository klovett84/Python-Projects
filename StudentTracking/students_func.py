...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_students (col_fname,col_lname,col_fullname,col_course,col_phone,col_email) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','Python','111-111-1111','jdoe@email.com'))
            conn.commit()
    conn.close()
...
def onSelect(self,event):
    ...
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_lname,col_course,col_phone,col_email FROM tbl_students WHERE col_fullname = (?)""", [value])
        ...
        for data in varBody:
            ...
            self.txt_course.insert(0,data[2])
