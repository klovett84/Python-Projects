...
def load_gui(self):
    ...
    self.txt_course.grid(row=5,column=0,rowspan=1,columnspan=2,padx=(30,40),pady=(0,0),sticky=N+E+W)
    ...
    self.btn_update = tk.Button(self.master,width=12,height=2,text='Update',command=lambda: students_func.onUpdate(self))
    ...
    self.btn_close = tk.Button(self.master,width=12,height=2,text='Close',command=lambda: students_func.ask_quit(self))
    self.btn_close.grid(row=10,column=4,columnspan=1,padx=(15,0),pady=(45,10),sticky=E)

    students_func.create_db(self)
    students_func.onRefresh(self)
